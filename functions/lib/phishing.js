...
exports.analyzePhishingProxy = void 0;
const https_1 = require("firebase-functions/v2/https");
const firebase_functions_1 = require("firebase-functions");
const gemini_1 = require("./utils/gemini");
exports.analyzePhishingProxy = (0, https_1.onCall)({
    region: "us-central1",
    ...
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Authentication is required to analyze phishing patterns.");
    }
    // 2. Input Sanitization & Validation
    const rawText = request.data?.text;
    if (!rawText || typeof rawText !== "string") {
        throw new https_1.HttpsError("invalid-argument", "A valid message text is required.");
    }
    const cleanText = rawText.trim().substring(0, 10000); // Sensible limit to prevent payload abuse
    if (cleanText.length === 0) {
        throw new https_1.HttpsError("invalid-argument", "Message content cannot be empty.");
    }
    firebase_functions_1.logger.info(`analyzePhishingProxy execution triggered by user ${request.auth.uid}`);
...
Please analyze this content and return the structured JSON report.`;
    try {
        ...
        firebase_functions_1.logger.error("AI Phishing analysis failed or returned invalid JSON. Falling back to rules.", err);
