...
exports.analyzeUrlProxy = void 0;
const https_1 = require("firebase-functions/v2/https");
const firebase_functions_1 = require("firebase-functions");
const vt_1 = require("./utils/vt");
const gemini_1 = require("./utils/gemini");
exports.analyzeUrlProxy = (0, https_1.onCall)({
    region: "us-central1",
    ...
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Authentication is required to run URL safety scans.");
    }
    // 2. Input Sanitization & Validation
    const url = request.data?.url?.trim();
    if (!url || typeof url !== "string" || url.length > 2048) {
        throw new https_1.HttpsError("invalid-argument", "A valid URL string under 2048 characters is required.");
    }
    firebase_functions_1.logger.info(`analyzeUrlProxy execution triggered by user ${request.auth.uid} for URL: ${url}`);
    ...
    let vtResult = null;
    try {
        vtResult = await (0, vt_1.checkUrlSecurityVT)(url);
    }
    catch (err) {
        firebase_functions_1.logger.error("Failed to query VirusTotal inside analyzeUrlProxy:", err);
    }
...
Please evaluate these signals. Return a structured JSON response matching the schema.`;
    try {
        ...
        firebase_functions_1.logger.error("AI URL analysis failed or returned malformed JSON. Falling back to deterministic rules.", err);
        ...
        if (!hasHttps)
            fallbackIndicators.push("Insecure connection (HTTP)");
        if (isIp)
            fallbackIndicators.push("Numeric IP host instead of domain name");
        if (typosquattingDetected)
            fallbackIndicators.push("Potential typosquatting swap flagged");
        if (foundKeywords.length > 0)
            fallbackIndicators.push(`Contains potential spoof keywords: ${foundKeywords.join(", ")}`);
