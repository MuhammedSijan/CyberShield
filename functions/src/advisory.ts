...
export const securityAdvisoryProxy = onCall<AdvisoryRequest>(
  {
    region: "us-central1",
  ...
  async (request) => {
    ...
    if (!request.auth) {
      throw new HttpsError("unauthenticated", "Authentication is required to view security briefings.");
    }
    ...
    if (score < 0 || score > 100) {
      throw new HttpsError("invalid-argument", "Security score must be a number between 0 and 100.");
    }
    if (!Array.isArray(rawScans)) {
      throw new HttpsError("invalid-argument", "Scans parameter must be an array.");
    }

    // Limit array to prevent excessive prompt lengths (e.g. max 15 recent scans)
    const scansList = rawScans.slice(0, 15).map((s: any) => ({
    ...
    logger.info(`securityAdvisoryProxy triggered by user ${request.auth.uid} with ${scansList.length} scans`);
...
Please review this history and generate the personalized advisory report in structured JSON format.`;
    ...
    try {
      ...
      logger.error("AI Posture advisory generation failed. Returning fallback.", err);
