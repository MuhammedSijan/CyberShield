 ...
 */
export async function checkUrlSecurityVT(url: string): Promise<VtScanResult> {
  const apiKey = process.env.VIRUSTOTAL_API_KEY;
  if (!apiKey) {
    logger.warn("VIRUSTOTAL_API_KEY is not configured on the server.");
  ...
  try {
    ...
    if (response.status === 404) {
      ...
      logger.info(`URL not found in VirusTotal database: ${url}. Attempting to submit for scanning.`);
    ...
    if (!response.ok) {
      ...
      logger.error(`VirusTotal URL lookup failed: Code ${response.status}. Details: ${errText}`);
    ...
    logger.error("Error calling VirusTotal URL API:", error);
 ...
 */
export async function checkFileHashVT(hash: string): Promise<VtScanResult> {
  const apiKey = process.env.VIRUSTOTAL_API_KEY;
  if (!apiKey) {
    return { success: false, maliciousCount: 0, suspiciousCount: 0, harmlessCount: 0, totalEngines: 0, error: "VirusTotal API key not configured" };
  }
  ...
  try {
    ...
    logger.error("Error calling VirusTotal File API:", error);
    return { success: false, maliciousCount: 0, suspiciousCount: 0, harmlessCount: 0, totalEngines: 0, error: error?.message || "Internal network error" };
  }
}
