  ...
  while (retries >= 0) {
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(`Gemini API returned error code ${response.status}: ${errText}`);
        
        if (response.status === 429 || response.status >= 500) {
          if (retries > 0) {
            retries--;
            logger.info(`Retrying Gemini request. Retries left: ${retries}. Backoff delay: ${delay}ms`);
      ...
      if (retries > 0) {
        retries--;
        logger.warn(`Gemini call error: ${err.message || err}. Retrying in ${delay}ms...`);
        await new Promise((resolve) => setTimeout(resolve, delay));
        delay *= 2;
        continue;
      }
      logger.error("All Gemini retries exhausted. Failed to resolve query:", err);
      throw err;
    }
  }
